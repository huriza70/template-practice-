...
---------------------------------------------------*/
	...
	function PageLoad() {	
		...
		setTimeout(function(){
			...
			$('body').append('<audio loop autoplay volume="0" id="audio"><source src="audio/good-night-160166.mp3" type="audio/mpeg" /></audio>');
	...
	function Showcase() {
		...
		if( $('#showcase-slider').length > 0 ){	
			...
			$('.footer-button').on('mousedown touchstart', function(event) {
				...
				event.preventDefault();
				$('.button-border span[data-hover]').each(function(){$(this).attr('data-hover', "HALTE GEDRÜCKT");});
			...
			$('.footer-button').on('mouseup touchend', function(event) {
				...
				var progress = $('.btn-hold-progress-bar');
				$(progress).stop();
				$('.button-border span[data-hover]').each(function(){$(this).attr('data-hover', "Drücke und halte");});;	
...
---------------------------------------------------*/	
	...
	function ContactMap() {	
		...
		if( jQuery('#map_canvas').length > 0 ){					
			var latlng = new google.maps.LatLng(52.4005613,9.7794613);
			var settings = {
				zoom: 14,
				center: new google.maps.LatLng(52.4005613,9.7794613),
			...
			var contentString = '<div id="content-map-marker" style="text-align:center; padding-top:10px; padding-left:10px">'+
				'<div id="siteNotice">'+
				'</div>'+
				'<h4 id="firstHeading" class="firstHeading" style="color:#000!important; font-weight:600; margin-bottom:0px;">Hallo!</h4>'+
				'<div id="bodyContent">'+
				'<p color:#999; font-size:14px; margin-bottom:10px">Hier findest Du uns auf der Karte.</p>'+
				'</div>'+
				'</div>';
			...
			var companyPos = new google.maps.LatLng(52.4005613,9.7794613);	
